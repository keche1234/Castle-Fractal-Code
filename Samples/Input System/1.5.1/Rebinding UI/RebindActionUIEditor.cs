...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUIEditor : UnityEditor.Editor
    {
        ...
        protected void OnEnable()
        {
            ...
            m_ControlPresetProperty = serializedObject.FindProperty("m_CurrentControlPreset");
            ...
            m_actionOverRideStringProperty = serializedObject.FindProperty("m_ActionLabelStrings");
        ...
        protected void RefreshPresetOptions()
        {
            ...
            m_ControlSchemes = new GUIContent[possibleGroups.Length];
            m_ControlSchemeValues = new string[possibleGroups.Length];
            m_SelectedControlScheme = -1;

            string currentSchemeString = m_ControlPresetProperty.stringValue;
            ...
            for (int i = 0; i < possibleGroups.Length; i++)
            {
                ...
                if (currentSchemeString == possibleGroups[i])
                    m_SelectedControlScheme = i;
            }
        }

        protected void RefreshPresetOptions(int i)
        {
            RefreshBindingOptions();
            var actionReference = (InputActionReference)m_ActionProperty.objectReferenceValue;
            var action = actionReference?.action;
            InputControlScheme[] controlSchemes = action.actionMap.controlSchemes.ToArray();
