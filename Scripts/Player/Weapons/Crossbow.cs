    ...
    public override IEnumerator Attack()
    {
        ...
        while (chain)
        {
            ...
            for (int i = 0; i < (2 + chainNum); i++)
            {
                ...
                arrows[i].Setup(12 * range, owner, true, owner.GetCustomWeapon().GetPower(), 0.5f, (1f / 3) + (chainNum / 9f));
    ...
    public override IEnumerator Signature()
    {
        ...
        for (float i = 0; i < (sigActiveTime * duration); i += Time.deltaTime)
        {
            if (delay >= (6f / 60))
            {
                ...
                arrow.Setup(18, owner, true, owner.GetCustomWeapon().GetPower() * damage, -1, 1, 1, true);
